...
class TemplateServiceTest {
  ...
  void getDefaultGermanDataProtectionTemplate_Should_ReturnTemplateDTO()
    ...
    assertThat(
        ...
        .contains("<#if name?exists>${name}<br/></#if><#if postCode?exists>postcode: ${postCode}<br/></#if><#if city?exists> city: ${city}<br/></#if><#if phoneNumber?exists>phoneNumber: ${phoneNumber}<br/></#if><#if email?exists>email: ${email}</#if>");
    assertThat(
            template
                .getAgencyContext()
                .getDataProtectionOfficer()
